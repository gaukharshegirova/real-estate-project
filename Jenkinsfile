pipeline {
    ...
    environment {
        ...
        ANS_KEYPAIR="real-estate-project"
        AWS_ACCOUNT_ID=sh(script:'export PATH="$PATH:/usr/local/bin" && aws sts get-caller-identity --query Account --output text', returnStdout:true).trim()
        ECR_REGISTRY="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com"
        APP_REPO_NAME = "real-estate-project"
        ...
        stage('wait the instance') {
            steps {
                script {
                    ...
                    id = sh(script: 'aws ec2 describe-instances --filters Name=tag-value,Values=real_estate_project Name=instance-state-name,Values=running --query Reservations[*].Instances[*].[InstanceId] --output text',  returnStdout:true).trim()
